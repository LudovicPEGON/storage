...
import com.everteam.storage.domain.ESRepository.TypeEnum;
import com.everteam.storage.web.rest.FilesApi;
import com.everteam.storage.web.rest.RepositoriesApi;
...
public class DriveClientTest {
    ...
    public  void init() throws IOException, GeneralSecurityException {
        if (repositoryId == null) {
            List<ESRepository> repositories = repositoryApiClient.listRepositories().getBody();
        ...
        ESFile folder = fileApiClient.createFile(repositoryId, null, name.getMethodName(), name.getMethodName() + "_"+ OffsetDateTime.now().toString()).getBody();
    ...
    public void testChildren() throws IOException {
        createFile();
        createFolder();
        ESFileList files = fileApiClient.getFileChildren(folderId, false, false, 100).getBody();
        // Assert
    ...
    public void testdownloadTo() throws IOException {
        ...
        byte[] bytes = fileApiClient.getFileContent(fileId).getBody();
    ...
    public void testGetPermissions() throws IOException {
        List<ESPermission> permissions = fileApiClient.getFilePermissions(folderId).getBody();
    ...
    public void testGetFile() throws IOException {
        ...
        String fileId = createFile();
        ESFile file = fileApiClient.getFile(fileId, true, true).getBody();
    ...
    private String createFile() throws IOException {
        return  fileApiClient.createFile(folderId, FILE, FILE.getName(), "FDescription_" + CODE).getBody().getId();
    }
    
    private String createFolder() throws IOException {
        return fileApiClient.createFile(folderId, null, "DName_"+  CODE, "DDescription_" + CODE).getBody().getId();
    }
