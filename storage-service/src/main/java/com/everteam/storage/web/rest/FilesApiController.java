...
public class FilesApiController implements FilesApi {
    ...
    public ResponseEntity<Void> checkUpdates(
            @PathVariable("id") String encryptedFileId,
        ...
        } catch (IOException | GeneralSecurityException | SevereException | ExecutionException e) {
            throw new WebApplicationException(e, Status.INTERNAL_SERVER_ERROR);
        }
    }

    @Override
    public ResponseEntity<ESFile> copyFile(
            @PathVariable("id") String encryptedFileId,
            @RequestParam(value = "targetId", required = true) String encryptedTargetId) {
        try {
            ESFileId fileId = new FileIdConverter().convert(encryptedFileId);
            ESFileId targetId = new FileIdConverter()
                    .convert(encryptedTargetId);
            ESFileId result = fileService.copy(fileId, targetId);

            return new ResponseEntity<ESFile>(
                    fileService.getFile(result, false, false), HttpStatus.OK);
    ...
    @Override
    public ResponseEntity<ESFile> createFile(
            @PathVariable("id") String encryptedFileId,
            @RequestParam(value = "file", required = false) MultipartFile content,
        ...
        try {
            ...
            if (content == null) {
                if (name != null && !name.isEmpty()) {
                    newFileId = fileService.createFolder(fileId, name,
                            description);
                } else {
    ...
    @Override
    public ResponseEntity<ESFileList> getFileChildren(
            @PathVariable("id") String encryptedFileId,
        ...
        try {
            ...
            ESFileId fileId = new FileIdConverter().convert(encryptedFileId);
            ESFileList efl = fileService.getChildren(fileId, getPermissions,
                    getChecksum, maxResult);
    ...
    @Override
    public ResponseEntity<ESFile> getFile(
            @PathVariable("id") String encryptedFileId,
            @RequestParam(value = "getPermissions", required = false, defaultValue = "false") Boolean getPermissions,
            @RequestParam(value = "getChecksum", required = false, defaultValue = "false") Boolean getChecksum) {
        try {
            ESFileId fileId = new FileIdConverter().convert(encryptedFileId);
            ESFile file = fileService.getFile(fileId, getPermissions,
                    getChecksum);
    ...
    @Override
    public ResponseEntity<List<ESPermission>> getFilePermissions(
            @PathVariable("id") String encryptedFileId) {
        try {
            ESFileId fileId = new FileIdConverter().convert(encryptedFileId);
            List<ESPermission> permissions = fileService.getPermissions(fileId);
            return new ResponseEntity<List<ESPermission>>(permissions,
                    HttpStatus.OK);
        } catch (IOException | GeneralSecurityException e) {
            throw new WebApplicationException(e, Status.INTERNAL_SERVER_ERROR);
        }
    ...
    @Override
    public ResponseEntity<ESFile> moveFile(
            @PathVariable("id") String encryptedFileId,
            @RequestParam(value = "targetId", required = true) String encryptedTargetId) {
        try {
            ESFileId fileId = new FileIdConverter().convert(encryptedFileId);
    ...
    @Override
    public ResponseEntity<ESFile> updateFile(
            @PathVariable("id") String encryptedFileId,
        ...
        try {
            ...
            ESFileId fileId = new FileIdConverter().convert(encryptedFileId);
            FileInfo info = new FileInfo(content.getName(), description,
                    content.getContentType(), content.getSize(),
                    content.getInputStream());
