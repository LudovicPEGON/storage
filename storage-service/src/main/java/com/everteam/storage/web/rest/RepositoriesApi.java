...
public interface RepositoriesApi {
    ...
    @ApiOperation(value = "Get or Generate access token", notes = "get a valid access token or generate a new one ", response = String.class, tags = {
            "Administration", })
    @ApiResponses(value = {
    ...
    @RequestMapping(value = "/repositories/{id}/oauth2/token", produces = {
            "text/plain" }, method = RequestMethod.GET)
    default ResponseEntity<String> getRepositoryToken(
            @ApiParam(value = "Repository Id", required = true) @PathVariable("id") String id) {
        // do some magic!
        return new ResponseEntity<String>(HttpStatus.OK);
    }

    @ApiOperation(value = "Search repositories", notes = "you can search for available repositories in the storage service ", response = ESRepository.class, responseContainer = "List", tags = {
            "Repositories", })
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "search results matching criteria", response = ESRepository.class) })
    @RequestMapping(value = "/repositories", produces = {
