...
//            .getLogger(GoogleDrive.class);
    ...
    public ESFileList children(String parentId, boolean addPermissions,
        ...
        for (File f : files.getFiles()) {
            result.addItemsItem(
                    buildFile(drive, f, addPermissions, addChecksum));
        }
    ...
    public List<ESPermission> getPermissions(String fileId) throws IOException {
        PermissionList permissions = getDriveService().permissions()
                .list(fileId).execute();
    ...
    public void update(String fileId, FileInfo info) throws IOException {
        ...
        InputStreamContent mediaContent = new InputStreamContent(
                info.getMimeType(), info.getInputStream());
    ...
    public String insertFile(String parentId, FileInfo info)
            throws IOException {
        return insert(parentId, info.getName(), info.getMimeType(),
                info.getInputStream(), info.getDescription());
    }
    ...
    public Changes getChanges(String fileId, String token, int maxrows) throws IOException {
       ...
       /* Drive drive = getDriveService();
        Files.List request = drive.files().list().setPageSize(100)
                .setFields("*").setOrderBy("modifiedTime")
                .setQ("modifiedTime > '" + fromDate
    ...
    public String insert(String parentId, String name, String contentType,
            InputStream in, String description) throws IOException {
        // File's metadata.
        File body = new File();
        body.setName(name);
        ...
        if (in != null) {
            ...
            InputStreamContent mediaContent = new InputStreamContent(
                    contentType, in);
            file = getDriveService().files().create(body, mediaContent)
                    .execute();
        } else {
            // it's a folder
    ...
    private ESFile buildFile(Drive drive, File file, boolean addPermissions,
            boolean addChecksum) throws IOException {
        ESUser lastModifiedUser = null;
        User lastModifyingUser = file.getLastModifyingUser();
        if (lastModifyingUser != null) {
            lastModifiedUser = new ESUser()
                    .id(lastModifyingUser.getEmailAddress())
                    .displayName(lastModifyingUser.getDisplayName());
        }

        ESFile esfile = new ESFile().id(file.getId())
                .creationTime(OffsetDateTime
                        .parse(file.getCreatedTime().toStringRfc3339()))
                .description(file.getDescription())
                .directory(file.getMimeType()
                        .equals("application/vnd.google-apps.folder"))
                .fileSize(file.getSize()).lastAccessTime(null)
                .lastModifiedTime(OffsetDateTime
                        .parse(file.getModifiedTime().toStringRfc3339()))
                .lastModifiedUser(lastModifiedUser).mimeType(file.getMimeType())
                .name(file.getName()).owners(null);
        ...
        if (addPermissions) {
            List<ESPermission> permissions = buildPermissions(
                    file.getPermissions());
        ...
        if (parents != null) {
            for (String parent : parents) {
                esfile.addParentsItem(
                        new ESParent().id(parent).paths(absPath(drive, file)));
            }
        }
        return esfile;
    }
    ...
    private List<ESPermission> buildPermissions(List<Permission> permissions) {
        ...
        if (permissions != null) {
            for (Permission p : permissions) {
                permissionss.add(new ESPermission()
                        .accountType(AccountTypeEnum.fromValue(p.getType()))
                        .domain(p.getDomain())
                        .addRolesItem(RolesEnum.fromValue(p.getRole()))
    ...
    private List<String> absPath(Drive drive, File file) throws IOException {
        ...
        while (true) {
            ...
            if (parent.getParents() == null || parent.getParents().size() == 0
                    || parent.getId()
                            .equals(getRepository().getRootDirectory())) {
                break;
            }
