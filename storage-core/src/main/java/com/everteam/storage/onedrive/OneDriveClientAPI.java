...
public class OneDriveClientAPI {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(OneDriveClientAPI.class);
    ...
    public ESFileList children(String parentId, boolean addPermissions,
            boolean addChecksum, int maxSize) {
        ...
        if (parentId != null && !parentId.isEmpty()) {
            response = exchangeUri(
                    builder.path(ITEM_CHILDREN_URL).build(parentId));
        } else {
        ...
        if (HttpStatus.OK.equals(response.getStatusCode())
            ...
            try {
                ...
                for (int i = 0; i < array.length(); i++) {
                    ESFile file = getESFileFromJSONObject(
                            array.getJSONObject(i), addChecksum);
                    if (addPermissions) {
                        try {
    ...
    public void downloadTo(String fileId, OutputStream outputstream)
            throws IOException {
        // This call was the one documented for OneDrive but the stream returned
        // is not well formatted
        // https://dev.onedrive.com/items/download.htm
        // URI uri =
        // UriBuilder.fromPath(BASE_URL).path(GET_ITEM_CONTENT).build(fileId.getRelativeId());
        ...
        // org.apache.tomcat.util.http.fileupload.IOUtils.copy(inputStream,
        // outputstream);
        // }
        // }
        ...
        // org.apache.tomcat.util.http.fileupload.IOUtils.copy(is,
        // outputstream);
        // }
        // }
            ...
            try {
                ...
                if (obj.has(Item.FILE)) {
                    try (InputStream is = getFileInputStream(
                            new URI(obj.getString(Item.DOWNLOAD_URL)))) {
    ...
    public String insertFolder(String parentId, String name, String description)
        ...
        URI uri = null;
        if (parentId != null && !parentId.isEmpty()) {
            uri = UriBuilder.fromPath(BASE_URL).path(ITEM_CHILDREN_URL)
                    .build(parentId);
        } else {
        ...
        try {
            ...
            String response = exchangeUri(uri, HttpMethod.POST, null, folder)
                    .getBody();
    ...
    public String insertFile(String parentId, FileInfo info)
            throws IOException {
        /*
         * https://dev.onedrive.com/items/upload.htm There are two upload mode
    ...
    public List<ESPermission> getPermissions(String fileId) throws IOException {
        URI uri = UriBuilder.fromPath(BASE_URL).path(ITEM_PERMISSIONS_URL)
                .build(fileId);
        ...
        if (result != null) {
            ...
            try {
                ...
                JSONArray array = obj.getJSONArray("value");
                for (int i = 0; i < array.length(); i++) {
                    permissions.add(getESPermissionFromJSONObject(
    ...
    public void update(String fileId, FileInfo info) throws IOException {
        ...
        // as OneDrive doesn't handle description we'll never update just the
        // metadata
        ESFile file = getFile(fileId);
    ...
    public void checkUpdates(String fileId, OffsetDateTime fromDate,
            Consumer<ESFile> consumer) throws IOException {
        /*
         * https://dev.onedrive.com/items/view_delta.htm#http-request GET
         * /drive/root/delta
         * 
         * Your app begins by calling delta without any parameters. The service
         * starts enumerating the folder's hierarchy, returning pages of items
         * and either an @odata.nextLink or an @odata.deltaLink, as described
         * above. Your app should continue calling delta either with
         * the @odata.nextLink until you no longer see an @odata.nextLink
         * returned, or you see a response with an empty set of changes. After
         * you have finished receiving all the changes, you may apply them to
         * your local state.
         * 
         */
                ...
                try {
                    ...
                    for (int i = 0; i < values.length(); i++) {
                        ...
                        if (!subobj.has(Item.ROOT)) {
                            ...
                            // creation or last modification > fromDate ; ie
                            // we've to take this into account
                            if (file.getCreationTime().isAfter(fromDate) || file
                                    .getLastModifiedTime().isAfter(fromDate)) {
                                // if there is a fileId we only take file/folder
                                // which have this for parent
                                if (fileFilter != null) {
                                    if (fileFilter.getDirectory()) {
                                        Path fileFilterPath = Paths.get(
                                                fileFilter.getParents().get(0)
                                                        .getPaths().get(0),
                                                fileFilter.getName());
                                        String filePath = file.getParents()
                                                .get(0).getPaths().get(0);
    ...
    private String simpleItemUpload(String parentId, FileInfo info) {
        /*
         * https://dev.onedrive.com/items/upload_put.htm PUT
         * /drive/items/{parent-id}/children/{filename}/content Content-Type:
         * text/plain
         * 
        ...
        ESFile file = null;
        if (parentId != null && !parentId.isEmpty()) {
            uri = UriBuilder.fromPath(BASE_URL).path(ITEM_SIMPLE_UPLOAD_URL)
                    .build(parentId, info.getName());
        } else {
            uri = UriBuilder.fromPath(BASE_URL).path(ROOT_SIMPLE_UPLOAD_URL)
                    .build(info.getName());
        }
        try {
            ...
            ResponseEntity<String> response = exchangeUri(uri, HttpMethod.PUT,
                    headers, result.toByteArray());
    ...
    protected static final class Item {
         ...
         * { "createdBy": { "user": { "id":
         * "8dd5c724-7035-48a5-9481-0ca40cbe7318", "displayName": "Kader Bennat"
         * } }, "createdDateTime": "2017-03-22T09:21:24Z", "eTag":
         * "\"{D9AEA7C3-98BA-4221-AF8F-8DD0C3A9FD87},1\"", "id":
         * "012YP5EGGDU6XNTOUYEFBK7D4N2DB2T7MH", "lastModifiedBy": { "user": {
         * "id": "8dd5c724-7035-48a5-9481-0ca40cbe7318", "displayName":
         * "Kader Bennat" } }, "lastModifiedDateTime": "2017-03-22T09:21:24Z",
         * "name": "Test TTR", "webUrl":
         * "https://everteamsoftware365-my.sharepoint.com/personal/k_bennat_everteamsoftware365_onmicrosoft_com/Documents/Test%20TTR",
         * "cTag": "\"c:{D9AEA7C3-98BA-4221-AF8F-8DD0C3A9FD87},0\"",
         * "fileSystemInfo": { "createdDateTime": "2017-03-22T09:21:24Z",
         * "lastModifiedDateTime": "2017-03-22T09:21:24Z" }, "folder": {
         * "childCount": 0 }, "parentReference": { "driveId":
         * "b!kkEY-OuMtUyR3G9XJuVtiYSU-chcIHpLp6x9vXqZOxvhr2cySdbmTaywovS6o4if",
         * "id": "012YP5EGF6Y2GOVW7725BZO354PWSELRRZ", "path": "/drive/root:" },
         * "size": 0 }
         */
    ...
    private ESFile getESFileFromJSONObject(JSONObject jsonObject,
        ...
        try {
            ...
            OffsetDateTime created = OffsetDateTime
                    .parse(jsonObject.getString(Item.CREATION_TIME));
            OffsetDateTime lastModified = OffsetDateTime
                    .parse(jsonObject.getString(Item.LAST_MODIFIED_DATE));
            boolean isFolder = jsonObject.has(Item.FOLDER);
            Long size = jsonObject.getLong(Item.SIZE);

            JSONObject parentReference = jsonObject
                    .getJSONObject(Item.PARENT_REFERENCE);
            ...
            file.id(id).name(name).creationTime(created).directory(isFolder)
                    .fileSize(size).lastModifiedTime(lastModified)
                    .mimeType(fileNameMap.getContentTypeFor(name))
                    .addParentsItem(new ESParent().id(parentId)
                            .addPathsItem(parentPath));
            ;
            // we don't have it in delta cf checkUpdates method
            if (jsonObject.has(Item.LAST_MODIFIED_BY)) {
                JSONObject lastModifiedBy = jsonObject
                        .getJSONObject(Item.LAST_MODIFIED_BY)
                        .getJSONObject(User.TAG_NAME);
                String lastModifiedById = lastModifiedBy.getString(User.ID);
                String lastModifiedByName = lastModifiedBy
                        .getString(User.DISPLAY_NAME);
                file.lastModifiedUser(new ESUser().id(lastModifiedById)
                        .displayName(lastModifiedByName));
            }

            if (!isFolder && addChecksum) {
                downloadUrl = jsonObject.getString(Item.DOWNLOAD_URL);
                md5 = DigestUtils.md5DigestAsHex(
                        getFileInputStream(new URI(downloadUrl)));
                file.checksum(md5);
            }
        } catch (JSONException e) {
            LOG.error(e.getMessage(), e);
    ...
    protected static final class Permission {
        /*
         * A permission JSON Object { "id":
         * "OEJBNjA4Q0MtN0Q2MC00RjEzLUEyNzItOEIzQUI5M0ZCOEQ3", "roles": [
         * "write" ], "link": { "scope": "anonymous", "type": "edit", "webUrl":
         * "https://everteamsoftware365-my.sharepoint.com/personal/k_bennat_everteamsoftware365_onmicrosoft_com/_layouts/15/guestaccess.aspx?docid=12bc432bfa4d743aba82418ac56ba1299&authkey=ARzol0EQgoPw5agvXMU4DLg"
         * } }, { "grantedTo": { "user": { "id":
         * "8dd5c724-7035-48a5-9481-0ca40cbe7318", "displayName": "Kader Bennat"
         * } }, "id":
         * "aTowIy5mfG1lbWJlcnNoaXB8ay5iZW5uYXRAZXZlcnRlYW1zb2Z0d2FyZTM2NS5vbm1pY3Jvc29mdC5jb20",
         * "roles": [ "write" ] }
         */
        public static final String ID = "id";
    ...
    private ESPermission getESPermissionFromJSONObject(JSONObject jsonObject) {
        ...
        try {
            ...
            if (jsonObject.has(Permission.GRANTED_TO)) {
                JSONObject grantedTo = jsonObject
                        .getJSONObject(Permission.GRANTED_TO)
                        .getJSONObject(User.TAG_NAME);
    ...
    protected static final class Delta {
        /*
         * A delta object { "@odata.context":
         * "https://graph.microsoft.com/v1.0/$metadata#Collection(driveItem)",
         * "@odata.deltaLink":
         * "https://graph.microsoft.com/v1.0/users('8dd5c724-7035-48a5-9481-0ca40cbe7318')/drive/root/microsoft.graph.delta(token='1;%234;%231;3;3267afe1-d649-4de6-acb0-a2f4baa3889f;636262901470930000;53088938;%23')",
         * "value": [Item] }
         * 
         */
