...
/**
 * Utility class for OAuth2 authentification based on google oauth2 <br>
 *
 * The first step is to call loadCredential(String) based on the known user ID
 * to check if the end-user's credentials are already known.<br>
 * If not, call newAuthorizationUrl() and direct the end-user's browser to an
 * authorization page.<br>
 * The web browser will then redirect to the redirect URL with a "code" query
 * parameter which can then be used to request an access token using
 * newTokenRequest(String).<br>
 * Finally, use createAndStoreCredential(TokenResponse, String) to store and
 * obtain a credential for accessing protected resources.<br>
 *
 *
 */
...
public class OAuth2Utils {
    ...
    private AuthorizationCodeFlow.Builder getAuthorizationCodeFlowBuilder(
            IDrive drive) throws GeneralSecurityException, IOException {
        Builder builder = new AuthorizationCodeFlow.Builder(
                BearerToken.authorizationHeaderAccessMethod(),
                GoogleNetHttpTransport.newTrustedTransport(),
                JacksonFactory.getDefaultInstance(),
                new GenericUrl(authDetails.getAccessTokenUri()),
                new ClientParametersAuthentication(authDetails.getClientId(),
                        authDetails.getClientSecret()),
                authDetails.getClientId(),
                authDetails.getUserAuthorizationUri());
        builder.setCredentialDataStore(StoredCredential.getDefaultDataStore(
                new FileDataStoreFactory(new File(CREDENTIAL_REPOSITORY,
                        drive.getRepository().getId()))));
        return builder;
    }
    ...
    public HttpHeaders newAuthorization()
        ...
        AuthorizationCodeRequestUrl url = authorizationCodeFlow
                .newAuthorizationUrl();
    ...
    private String getCallbackURL() {
        return UriBuilder.fromPath(front).path(CALLBACK_URL).build().toString();
    }

    public Credential createAndStoreCredential(String authorizationCode)
            throws IOException {
        TokenResponse tokenResponse = authorizationCodeFlow
                .newTokenRequest(authorizationCode)
                .setRedirectUri(getCallbackURL()).execute();
        return authorizationCodeFlow.createAndStoreCredential(tokenResponse,
                authDetails.getClientId());
    }
