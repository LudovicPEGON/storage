...
public class ESRepositoryDeserializer extends JsonDeserializer<ESRepository> {
    ...
    public ESRepositoryDeserializer(
    ...
    public ESRepository deserialize(JsonParser p, DeserializationContext ctxt)
        ...
        try {
            ESRepository repository = (ESRepository) defaultDeserializer
                    .deserialize(p, ctxt);
            ESFileId repositoryFid = new ESFileId(
                    Encryptor.decrypt(repository.getId()));
            repository
                    .setId(Encryptor.decrypt(repositoryFid.getRepositoryId()));
            return repository;
        } catch (Exception e) {
